...
def comment(pname):
    ...
    if comment.validate_on_submit():
        comment=Comment(comment=comment.comment.data,pitch_id=pitch.id,user_id=current_user.id)
        db.session.add(comment)
        db.session.commit()
        return redirect(url_for('main.comment',pname=pname))
    
    return render_template('pitch.html',comment=comment,pitch=pitch,comments=comment_query,title='Pitch Comment',image=image)
...
def like_action(comment_id, action):
    comment = Comment.query.filter_by(id=posted).first_or_404()
    if action == 'vote':
